...
ttt=2000
SS=1;
source("/home/llin/discrete_model_final/test_model/for_pilot_Exp.R")
dyn.load("/home/llin/discrete_model_final/test_model/updategammak_noPu_Exp.so")
dyn.load("/home/llin/discrete_model_final/test_model/updatealphau_noPu_Exp.so")
dyn.load("/home/llin/discrete_model_final/test_model/updatealphas_Exp.so")
...
ttt=2000
SS=1;
source("/home/llin/discrete_model_final/test_model/for_CC_Exp.R")
dyn.load("/home/llin/discrete_model_final/test_model/updategammak_noPu_Exp.so")
dyn.load("/home/llin/discrete_model_final/test_model/updatealphau_noPu_Exp.so")
dyn.load("/home/llin/discrete_model_final/test_model/updatealphas_Exp.so")
...
for (tt in 2:T) {
       ...
       #update gamma
       res1 <- .Call("updategammak_noPu_Exp",n_s = n_s,n_u=n_u,gammat = gamma[,,tt-1],I=I,K=K,SS = SS,alphau = alpha_u[tt,],alphas = alpha_s[tt-1,],alpha=1,mk=mk,Istar = Istar,
...
for (stt in 1:sTT) {
   for (tt in 2:T) {
       # update alphau
       res2 <- .Call("updatealphau_noPu_Exp", alphaut = alpha_u[tt-1,],n_s = n_s,n_u=n_u, I=I, K=K, lambda_u = lambda_u, var_p = varp_u, ttt = ttt,gammat =gamma[,,tt-1],sigma_u=sigma_u )
       alpha_u[tt,] = res2$alphau_tt;
       A_alphau[,tt] = res2$Aalphau;
       ...
       # update alphas
       res3 <- .Call("updatealphas_Exp", alphast = alpha_s[tt-1,], n_s = n_s,  K=K, I=I, lambda_s = lambda_s, gammat =gamma[,,tt], var_1 = varp_s1,var_2 = varp_s2,p_var = pvar_s,ttt = ttt,sigma_s=sigma_s)
       alpha_s[tt,] = res3$alphas_tt;
       A_alphas[,tt] = res3$Aalphas;
       cat("iteration = ", iter <- tt, "\n")
    }
